...
Нашли баг (замечание - нет template с указанием версии):

![alt text](image-16.png)

`git checkout -b terraform-05`
`git add providers.tf`
`git commit -m "feat: init terraform-05 (missing template version)"`
`git push -u origin terraform-05`

`git checkout -b terraform-hotfix`

Редактируем код - фиксим баг.
...
`terraform validate`
`git add providers.tf`
`git commit -m "fix: added missing template provider version"`
...
- Проверяем в diff и делаем pull request в github репозитории:

![alt text](image-20.png)

Добавляем комментарий и делаем Create Pull Request.

![alt text](image-21.png)

Ссылка на Pull Request:
https://github.com/jeeneducation87/ter-homeworks/pull/1
...
- Написание кода. Создаем validation.tf:

![alt text](image-22.png)

`terraform console`

# Проверка валидного ip (должен вернуть значение):
var.test_ip_address

![alt text](image-23.png)

# Попытка присвоить неверный адрес (должен вернуть ошибку):

can(regex("^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$", "1920.1680.0.1"))
Результат: false

![alt text](image-24.png)

- Проверка ошибки (Invalid IP):

`terraform plan -var='test_ip_address=1920.168.0.1'`
`terraform plan -var='test_ip_address=10.0.0.5'`

![alt text](image-25.png)

![alt text](image-27.png)

- Проверка ошибки (Invalid List):
